...
\begin{prop}
  $V$, $W$を$\KK^n$の部分空間とする.
  $\aaa=(a_i)_{i\in I}\in\KK^n$に対し,
  $\KK^{2n}$の元$\widetilde \aaa$, $\widehat \aaa$, $\widecheck \aaa$を
  \begin{align*}
   \widetilde\aaa&=\begin{pmatrix}a_1\\\vdots\\a_n\\a_1\\\vdots\\a_n\end{pmatrix},
   &\widehat\aaa&=\begin{pmatrix}a_1\\\vdots\\a_n\\0\\\vdots\\0\end{pmatrix},
   &\widecheck\aaa&=\begin{pmatrix}0\\\vdots\\0\\a_1\\\vdots\\a_n\end{pmatrix}
  \end{align*}
  で定義し,
  $\KK^{2n}$の部分空間$\widetilde V$と$\widehat W$を次で定義する:
  \begin{align*}
    \widetilde V&=\Set{\widetilde\aaa| \aaa\in V}
    &\widehat W&=\Set{\widehat\aaa| \aaa\in W}
  \end{align*}
  このとき,
  \begin{align*}
    W\cap V = \Set{\aaa \in \KK^n | \widecheck\aaa \in \widetilde V\cap \widehat W}.
  \end{align*}
\end{prop}
\begin{proof}\end{proof}
...
\item[Proceedure]\makebox{}
  \begin{enumerate}
  \item
    $\widetilde A=\Set{\begin{pmatrix}\aaa_1\\\aaa_1\end{pmatrix},\ldots,\begin{pmatrix}\aaa_n\\\aaa_n\end{pmatrix}}$
    とする.
  \item
    $\widehat B=\Set{\begin{pmatrix}\bbb_1\\\zzero_m\end{pmatrix},\ldots,\begin{pmatrix}\bbb_l\\\zzero_m\end{pmatrix}}$
    とする.
  \item $X=\reduce(\widetilde A\cup \widehat B)$とする.
  \item $\widecheck X=\Set{\begin{pmatrix}\aaa\\\bbb\end{pmatrix}\in X|\aaa=\zzero_m}$とする.
  \item $\Set{\bbb|\begin{pmatrix}\zzero_m\\\bbb\end{pmatrix}\in\widecheck X}$を出力し終了.
  \end{enumerate}
\end{description}
\end{algorithm}
\begin{prop}
  \Cref{alg:intersection}は有限回のステップで終了し,
  $\KK^n$の有限部分集合を出力する.
\end{prop}

\begin{prop}
  \Cref{alg:intersection}の出力は,
